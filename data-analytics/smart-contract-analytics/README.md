...
Now let's create a key and download the json file. Enter your service account by clicking on its name. Go to `KEYS` tab then click on `ADD KEY` and `CREATE new key`.
...
Next, let's create a dataset to load data into! Click [here](https://console.cloud.google.com/bigquery) to go to your project! Click on the options next to your project then `Create dataset`.
![image](https://user-images.githubusercontent.com/78521141/193474561-85eeb77c-70f6-4669-b537-1b1fd650623a.png)

Now, name your dataset and set the location to store the data. I've selected Singapore because, well, I feel comforted that it's stored here in Singapore. But also, having it near you means it will also reduce latency. 
